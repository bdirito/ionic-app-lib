...
Login.requestLogIn = function requestLogin(email, password, saveCookies) {
  ...
  request({
    ...
    if (parseInt(response.statusCode, 10) !== 200) {
      return q.reject(chalk.red('Email or Password incorrect. Please visit ') + chalk.white(settings.IONIC_DASH) + chalk.red(' for help.'));
    }
...
Login.requestSignup = function requestLogin(email, password, saveCookies) {
  var q = Q.defer();
  var transformedCookies = null;
