...
Serve.loadSettings = function loadSettings(argv, project) {
  ...
  if (project.get('https') || argv.httpscert || argv.httpskey) {
    options.https = project.get('https') || {};
    options.https.cert = argv.httpscert || options.https.cert;
    options.https.key = argv.httpskey || options.https.key;
    options.https.passphrase = argv.httpspassphrase || options.https.passphrase;
  }
  options.port = argv.port || argv.p || DEFAULT_HTTP_PORT;
  options.liveReloadPort = argv.livereloadport || argv.r ||
...
Serve.openBrowser = function openBrowser(options) {
  if (options.launchLab || options.launchBrowser) {
    ...
    var openUrl = options.launchLab ?
          [Serve.host(options.address, options.port, options), IONIC_LAB_URL] :
          [Serve.host(options.address, options.port, options)];
...
Serve.runLivereload = function runLivereload(options, app) {
  ...
  try {
    ...
    options.liveReloadServer = Serve.host(options.address, liveReloadPort, options);
...
Serve.startServer = function startServer(options, app) {
  options.devServer = Serve.host(options.address, options.port, options);
  ...
  serverCreateFn = http.createServer;
  if (options.https) {
    if (! options.https.key ) {
      log.error('When using https you must specify a key');
    }
    if (! options.https.cert ) {
      log.error('When using https you must specify a certificate');
    }
    if (! options.https.passphrase ) {
       log.warn('No key passphrase specified for https mode server');
    }

    if (!fs.existsSync(options.https.key)) {
      log.error('Cannot find specified https key "' + options.https.key + '".');
    }
    if (!fs.existsSync(options.https.cert)) {
      log.error('Cannot find specified https certificate "' + options.https.cert + '".');
    }
    serverCreateFn = function(requestListener) {
      console.log('creating', requestListener);
  ...
  var server = serverCreateFn(function(req, res) {
  ...
  app.use(server);
  try {
    runningServer = app.listen(options.port);//, '0.0.0.0');//options.address);
    //runningServer = server.listen(options.port);//, '0.0.0.0');//options.address);
...
Serve.host = function host(address, port, options) {
  ...
  var hostAddress = ['http' + (options.https ? 's' : '') + '://', address, ':', port].join('');
  return hostAddress;
};
