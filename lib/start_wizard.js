var Q = require('q');
var http = require('http');
var connect = require('connect');
var serveStatic = require('serve-static');
var finalhandler = require('finalhandler');
var request = require('request');

var log = require('./logging').logger;
var Utils = require('./utils');

var Start = require('./start');
var Login = require('./login');

function getBody(req, cb) {
  var body = [];
  req.on('data', function(chunk) {
    body.push(chunk);
  }).on('end', function() {
    body = Buffer.concat(body).toString();
    cb(body);
  });
}

function handleApiCli(req, res) {
  getBody(req, function(body) {

    var d = JSON.parse(body);
    console.log('DOING CLI COMMAND', d);

    try {
      Start.startApp({
         appDirectory: d.app.directory,
         appName: d.app.name,
         packageName: d.app.id,
         isCordovaProject: true,
         template: d.app.template,
         targetPath: Utils.getProjectDirectory({ appDirectory: d.app.directory }),
         v2: true
      })
      .then(function() {
        res.writeHead(200, {'Content-Type': 'application/json'});
        res.end('{"status": "success", "data": ""}');
      })
      .catch(function(error) {
        res.writeHead(200, {'Content-Type': 'application/json'});
        res.end('{"status": "error", "data": "' + error.message + '"}');
        log.error(error);
      });
    } catch(e) {
      res.writeHead(200, {'Content-Type': 'application/json'});
      res.end('{"status": "error", "data": "' + e.message + '"}');
      log.error(e);
    }

  });

}

function handleApiEnv(req, res) {
  var appDirectory = process.cwd();
  res.writeHead(200, {'Content-Type': 'application/json'});
  res.end(JSON.stringify({
    cwd: appDirectory
  }));
}

function handleApiRun(req, res) {
  res.writeHead(200, {'Content-Type': 'application/json'});
  res.end(JSON.stringify({
    cwd: appDirectory
  }));
}

function handleApiLogin(req, res) {
  getBody(req, function(body) {

    var d = JSON.parse(body);
    console.log('DOING LOGIN', d);

    var promise;

    promise = Q({ email: d.email, password: d.password });

    return promise
    .then(function(loginInfo) {
      return Login.requestLogIn(d.email, d.password, true);
    })
    .then(function(cookieJar) {
      log.info('Logged in!');

      console.log('Jar', cookieJar);

      res.writeHead(200, {'Content-Type': 'application/json'});

      return cookieJar;
    })
    .catch(function(ex) {
      log.error(ex);
    });
  });
}

function handleApiSignup(req, res) {
  getBody(req, function(body) {

    var d = JSON.parse(body);
    console.log('DOING SIGNUP', d);

    var promise;

    promise = Q({ email: d.email, password: d.password });

    return promise
    .then(function(loginInfo) {
      return Login.requestSignup(d.email, d.password, true);
    })
    .then(function(cookieJar) {
      log.info('Logged in!');

      console.log('Jar', cookieJar);

      res.writeHead(200, {'Content-Type': 'application/json'});

      return cookieJar;
    }, function(signupRes) {
      res.writeHead(400, {'Content-Type': 'application/json'});
      res.end('{"status": "error", "data": ' + signupRes[1] + '}');

      console.log('Unable to sign up we got this body', signupRes[1]);
    })
    .catch(function(ex) {
      log.error(ex);
    });
  });
}


module.exports = {
  start: function() {
    var q = Q.defer();

    var app = connect();

    console.log('Starting Start Wizard From App Lib', __dirname);

    var serve = serveStatic(__dirname + '/assets/wizard');
    var server = http.createServer(function(req, res) {
      var done = finalhandler(req, res);

      if(req.url == '/api/env') {
        handleApiEnv(req, res);
        return;
      }
      if(req.url == '/api/cli') {
        handleApiCli(req, res);
        return;
      }
      if(req.url == '/api/run') {
        handleApiRun(req, res);
        return;
      }
      if(req.url == '/api/signup') {
        handleApiSignup(req, res);
        return;
      }
      if(req.url == '/api/login') {
        handleApiLogin(req, res);
        return;
      }

      serve(req, res, done);
    });

    // Listen
    app.use(server);
    try {
      runningServer = app.listen('8090', '0.0.0.0');
    } catch (ex) {
      Utils.fail('Failed to start the Ionic server: ' + ex.message);
    }

    log.info('√ Running dev server: ', chalk.cyan(options.devServer));

    return q.promise;
  }
}
